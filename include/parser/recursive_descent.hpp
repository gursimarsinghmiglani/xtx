...
inline std::unique_ptr<AST> Parser::parse_multiplicative_expr() {
    ...
    while (check(Token::TOK_MUL) || check(Token::TOK_DIV) || check(Token::TOK_DOT_MUL) || check(Token::TOK_DOT_DIV)) {
        auto ast = std::make_unique<AST>();
...
inline std::unique_ptr<AST> Parser::parse_postfix_expr() {
    ...
    while (1) {
        ...
        ast->children.push_back(std::move(left));
        if (right) ast->children.push_back(std::move(right));
        left = std::move(ast);
    }
    return left;
}
